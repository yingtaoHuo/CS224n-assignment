...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(embed_size, hidden_size, bias=True, dropout=dropout_rate, bidirectional=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]):
        ...
        enc_hiddens = pad_packed_sequence(output)[0]
        enc_hiddens = enc_hiddens.permute(1,0,2)
        last_hidden = self.h_projection(torch.cat((hn[0],hn[1]), dim=1))
        last_cell = self.c_projection(torch.cat((cn[0],cn[1]), dim=1))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###               as well as the new combined output  .
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_h, dim=2)), dim=2)
        ### END YOUR CODE
        #print('before:',e_t.data)
        # Set e_t to -inf where enc_masks has 1
        if enc_masks is not None:
            e_t.data.masked_fill_(enc_masks.bool(), -float('inf'))
        #print('after:',e_t.data)
